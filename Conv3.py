# -*- coding: utf-8 -*-
"""
Created on Thu Nov 17 17:16:56 2022
...
X = layers.Conv2D(16, 3, activation = 'relu', padding = 'same')(myInput)#(Num # filters, Window filter(w,w), activation("valid"` or `"same"`), strides(defult=1))
X = layers.MaxPooling2D((2, 2))(X)#(window(w,w), pading(defult))
X = layers.Dropout(20)(X)
X = layers.Conv2D(32, 3, activation = 'relu', padding = 'same')(X)
X = layers.MaxPooling2D((2, 2))(X) # we like a classifier but out put is picture
X = layers.Dropout(20)(X)
X= layers.Flatten()(X) # flatten layer change every inputs data to vectors
out_layer = layers.Dense(10, activation='softmax')(X) #last layer is classifier and use layer dense
...
plot_model(myModel, to_file='CNN_model3.png', show_shapes=True)#pdf
#==================================================
# Train our model
